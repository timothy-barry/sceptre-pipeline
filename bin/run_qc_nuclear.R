...
grna_odm_fp <- args[3]
response_n_umis_range_lower <- args[4]
response_n_umis_range_upper <- args[5]
response_n_nonzero_range_lower <- args[6]
response_n_nonzero_range_upper <- args[7]
p_mito_threshold <- args[8]
...
# process response_n_umis_range, response_n_nonzero_range, and p_mito_threshold
...
if (!identical(p_mito_threshold, "default")) args_to_pass[["p_mito_threshold"]] <- as.numeric(p_mito_threshold)

# call the qc function
gc() |> invisible()
sceptre_object <- do.call(what = sceptre:::run_qc_pt_1, args = args_to_pass)

# create plots
p1 <- sceptre::plot_covariates(sceptre_object)
p2 <- sceptre:::plot_cellwise_qc(sceptre_object)
...
ggplot2::ggsave(filename = "plot_cellwise_qc.png", plot = p2, device = "png",
                scale = 1.1, width = 4, height = 3, dpi = 330)
